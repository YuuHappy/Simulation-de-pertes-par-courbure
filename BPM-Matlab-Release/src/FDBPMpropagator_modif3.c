...
#pragma omp for schedule(dynamic)
    ...
    for (long i = 0;i < P->Nx * P->Ny;i++) {
        ...
        float x_proj = x * P->cosBendDirection + y * P->sinBendDirection;

        float x_caustic = P->R_eff * (P->n_eff / P->n_clad - 1.0f);

        // Safety: prevent invalid values
        if (!isfinite(x_caustic))
            x_caustic = 0.0f;

        // Width (never zero)
        float width = 4.0f * P->dx;
        if (width < 1e-12f)
            width = 1e-12f;

        float n_real = CREALF(n);

        // Safe tanh argument
        float arg = (fabsf(x_proj) - x_caustic) / width;

        // Clamp to prevent overflow
        if (arg > 20.0f) arg = 20.0f;
        if (arg < -20.0f) arg = -20.0f;

        float s = 0.5f * (1.0f + tanhf(arg));

        // Safe interpolation
        float n_mod = (1.0f - s) * n_real + s * P->n_eff;

        // HARD safety clamp
        if (!isfinite(n_mod) || n_mod < 1.0f || n_mod > 5.0f)
            n_mod = P->n_eff;

        // Compute n_bend safely
        float n_bend = n_mod *
            (1.0f - (n_mod * n_mod * x_proj / (2.0f * P->RoC * P->rho_e))) *
            (1.0f + x_proj / P->RoC);

        // FINAL CRITICAL PROTECTION
        if (!isfinite(n_bend))
            n_bend = P->n_0;
        ...
        /* ========================================================= */
        /* ========== ✅✅ NEW CAUSTIC PHYSICS END =================== */
        /* ========================================================= */

        // Multiplier (UNCHANGED except n_bend)
        floatcomplex a = P->multiplier[i] *
            CEXPF(P->d * (CIMAGF(n) + (n_bend * n_bend - P->n_0 * P->n_0) * I / (2 * P->n_0)));
