...
void applyMultiplier(struct parameters *P_global, long iz, struct debug *D) {
  ...
  for(long i=0;i<P->Nx*P->Ny;i++) {
    ...
    // Projection dans la direction de courbure
    float x_proj = x * P->cosBendDirection + y * P->sinBendDirection;

    // Position de la caustique
    float x_caustic = P->R_eff * (P->n_eff / P->n_clad - 1.0f);

    // Largeur de transition (optimale)
    float width = 4.0f * P->dx;

    // Indice local réel
    float n_real = CREALF(n);

    // Fonction lissée (tanh)
    float s = 0.5f * (1.0f + tanhf((fabsf(x_proj) - x_caustic) / width));

    // Mélange entre n réel et n_eff
    float n_mod = (1.0f - s) * n_real + s * P->n_eff;

    // Calcul de l'indice courbé
    float n_bend = n_mod *
        (1.0f - (sqrf(n_mod) * x_proj / (2.0f * P->RoC * P->rho_e))) *
        (1.0f + x_proj / P->RoC);
