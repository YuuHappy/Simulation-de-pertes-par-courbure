...
mexParameters = struct('dx',single(dx),'dy',single(dy),'dz',single(dz),'taperPerStep',single((1-P.taperScaling)/Nz),'twistPerStep',single(P.twistRate*P.Lz/Nz),...
  ...
  'inputPrecisePower',P.powers(end-length(zUpdateIdxs))*powerFraction,'xSymmetry',uint8(P.xSymmetry),'ySymmetry',uint8(P.ySymmetry), 'n_clad',single(P.n_clad), 'n_eff',single(P.n_eff));
